...
def oai2(request):
    ...
    if 'verb' in params:
        ...
        if verb == 'GetRecord':
            ...
            if 'metadataPrefix' in params:
                ...
                if len(metadata_prefix) == 1:
                    ...
                    if not MetadataFormat.objects.filter(prefix=metadata_prefix).exists():
                        errors.append(_error('cannotDisseminateFormat',
                                             metadata_prefix))
                    ...
                    errors.append(_error('badArgument_single',
            ...
            if 'resumptionToken' in params:
                ...
                paginator, headers, resumption_token, set_spec, metadata_prefix, from_timestamp, until_timestamp = _do_resumption_token(params, errors, header_list)
                ...
                if len(metadata_prefix) == 1:
                    ...
                    if not MetadataFormat.objects.filter(prefix=metadata_prefix).exists():
                        ...
                        from_timestamp, until_timestamp = _check_timestamps(params, errors)
                        if from_timestamp:
            ...
            if not Set.objects.all().exists():
                ...
                paginator, sets, resumption_token, set_spec, metadata_prefix, from_timestamp, until_timestamp = _do_resumption_token(params, errors, Set.objects.all())
            _check_bad_arguments(params, errors)
        else:
    ...
    return render(request, template if not errors else 'oai_pmh/error.xml',
                  locals(), content_type='text/xml')


def _check_bad_arguments(params, errors, msg=None):
    error_msg = 'The argument "%s" (value="%s") included in the request ' + \
                'is not valid.%s'

    for k, v in params.copy().items():
        errors.append({
            'code': 'badArgument',
            'msg': error_msg % (k, v, (' %s' % msg if msg else ''))
        })
        params.pop(k)
    ...
    if 'resumptionToken' in params:
        ...
        try:
            ...
            if timezone.now() > rt.expiration_date:
                errors.append(_error('badResumptionToken_expired.',
                                     resumption_token))
            else:
                if rt.set_spec:
                ...
                try:
                    ...
                    errors.append(_error('badResumptionToken',
                                         resumption_token))
        except ResumptionToken.DoesNotExist:
            paginator = Paginator(objs, NUM_PER_PAGE)
            page = paginator.page(1)
            errors.append(_error('badResumptionToken', resumption_token))
        _check_bad_arguments(params, errors, msg='The usage of resumptionToken as an argument allows no other arguments.')
    else:
        paginator = Paginator(objs, NUM_PER_PAGE)
        page = paginator.page(1)

    return (paginator, page, resumption_token, set_spec, metadata_prefix,
...
def _error(code, *args):
    if code == 'badArgument':
        return {
            'code': 'badArgument',
            'msg': ('The required argument "%s" is missing in the ' +
                'request.') % args[0]
        }
    elif code == 'badArgument_granularity':
        return {
            'code': 'badArgument',
            'msg': 'The granularity of the arguments "from" and "until" ' + \
                'do not match.'
        }
        ...
        return {
            'code': 'badArgument',
            'msg': 'The value "%s" of the argument "%s" is not valid.' % (
                args[0],
                args[1]
            )
        }
        ...
        return {
            ...
            'msg': 'The resumptionToken "%s" is invalid.' % args[0]
        }
    elif code == 'badResumptionToken_expired':
        return {
            'code': 'badResumptionToken',
            'msg': 'The resumptionToken "%s" is expired.' % args[0]
        }
        ...
        return {
            'code': 'badVerb',
            'msg': 'The verb "%s" provided in the request is illegal.' % args[0]
        }
    elif code == 'cannotDisseminateFormat':
        return {
            'code': 'cannotDisseminateFormat',
            'msg': ('The value of the metadataPrefix argument "%s" is not ' +
                    'supported.') % args[0]
        }
    elif code == 'idDoesNotExist':
        return {
            'code': 'idDoesNotExist',
            'msg': 'A record with the identifier "%s" does not exist.' % args[0]
        }
        ...
        return {
            'code': 'noMetadataFormats',
            'msg': 'There are no metadata formats available for the record' + \
                ' with identifier "%s".' % args[0]
        }
    elif code == 'noRecordsMatch':
        return {
            'code': 'noRecordsMatch',
            'msg': 'The combination of the values of the from, until, and ' +
